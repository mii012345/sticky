...
自分が作成・参加しているボードの一覧を表示する画面。

#### 表示条件

| 条件 | 表示対象 |
|------|----------|
| 自分が作成したボード | `createdBy` が自分の `clientId` と一致 |
| 自分が参加したボード | Participant テーブルに自分の `odclientId` が存在 |

※ 未ログイン状態（`clientId` がない場合）は空の一覧を表示

#### 画面構成

- ヘッダー: ロゴ、ユーザーアイコン（ログイン時）
- タイトルセクション: 「マイボード」、新規作成ボタン
- ボードグリッド: カード形式でボード一覧を表示
...
| 付箋移動 | グループ内の付箋を別のグループにドラッグ&ドロップで移動 |
...
付箋を削除すると、完全に消去されずゴミ箱に移動する。

#### ゴミ箱の特徴

- ヘッダー右側にゴミ箱アイコンを表示
- ゴミ箱にアーカイブされた付箋がある場合、バッジで件数を表示
- ゴミ箱アイコンをクリックするとダイアログが開く
...
| 2026-01-28 | ゴミ箱機能（アーカイブ・復元）を追加 |
| 2026-01-28 | グループ間の付箋移動機能を追加 |
| 2026-01-28 | トップページを「作成・参加したボードのみ表示」に変更 |
